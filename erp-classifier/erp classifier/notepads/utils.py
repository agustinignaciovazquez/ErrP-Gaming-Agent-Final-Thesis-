...
def get_separability_values(file, evoked_hits, evoked_no_hits, index):
    ...
    f.close()
    f1 = open("../notepads/maximos.txt", "a")
...
def group_and_average_hits_and_no_hits(epochs_data, labels, n):
    ...
    sub_index_hits = 0
    sub_index_no_hits = 0
    sum_n_hits = np.zeros(shape=(8,21))
    sum_n_no_hits = np.zeros(shape=(8,21))
...
def group_and_average_hits_and_no_hi(epochs_data, labels):
    ...
    new_epochs_data.append(sum_n_hits/sub_index_hits)
    new_labels.append(1)
    new_epochs_data.append(sum_n_no_hits/sub_index_no_hits)
    new_labels.append(0)


    return new_epochs_data, new_labels

def grid_lights_files_to_list(state_files):
    states = []
    for state_file in state_files:
        with open(state_file, "r") as inp:
            ...
            for line in inp:
                if 'x' in line:
                    dim = line.split("x")
                    states.append(int(dim[0]) * int(dim[1]))
                if '-' in line:
